...
document.addEventListener("DOMContentLoaded", function() {
    ...
    window.addEventListener('scroll', function() {
        if (window.scrollY > aboveHeight && !header.classList.contains('fixed')) {
            header.classList.add('fixed');
            header.style.animation = "stickymenu 500ms";
    ...
    if ($(window).scrollTop() > aboveHeight && !$('#header').hasClass('fixed')){
        if (window.scrollY > aboveHeight && !header.classList.contains('fixed')) {
            header.classList.add('fixed');
            header.style.animation = "stickymenu 300ms";
            header.style.animationFillMode = "forwards";
        }
    }
        ...
        entries.forEach((e) => {
            if(e.isIntersecting) {
                ...
                items.forEach(link => {
                    ...
                    console.log(e.target.id + " - " + link.dataset.nav);
    ...
    const rowObserver = new IntersectionObserver(entries => {
        entries.forEach(entry => {
            if (entry.isIntersecting && !entry.target.classList.contains('counter-stop')) {
                let frame = 0;
                const countTo = parseInt(entry.target.innerHTML.replace(/,/g, ''), 10);
                const counter = setInterval( () => {
                    frame++;
                    const progress = easeOutQuad( frame / totalFrames );
                    const currentCount = Math.round( countTo * progress );
                    if ( parseInt( entry.target.innerHTML, 10 ) !== currentCount ) {
                        entry.target.innerHTML = currentCount.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
                    }
                    if ( frame === totalFrames ) {
                        clearInterval( counter );
                        entry.target.classList.add('counter-stop');
                    }
                }, frameDuration );
            }
        });
    ...
    const numberRow = document.querySelectorAll('.counter');
